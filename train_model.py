...
# Configuration file
cfg_dir = './'
with open(os.path.join(cfg_dir, 'single_model.yaml')) as file:
...
# Box
box_data = pd.read_excel(cfg['data']['box_file'], index_col="img", dtype=list)
...
for step in ['train', 'test']:
    fold_data_step = pd.read_csv(os.path.join(cfg['data']['fold_dir'], '%s.txt' % step), delimiter=" ", index_col=1, header=None)
    fold_data_step.index = fold_data_step.index.rename('name')
    fold_data_step = fold_data_step.rename(columns={2: 'label'})
    fold_data[step] = fold_data_step

# Data Loaders
datasets = {step: util_data.Dataset(data=fold_data[step], classes=classes, boxes=boxes, img_dir=cfg['data']['img_dir'], step=step, img_dim=cfg['data']['img_dim']) for step in ['train', 'test']}
data_loaders = {'train': torch.utils.data.DataLoader(datasets['train'], batch_size=cfg['data']['batch_size'], shuffle=True, num_workers=num_workers, worker_init_fn=util_data.seed_worker),
                'val': torch.utils.data.DataLoader(datasets['test'], batch_size=cfg['data']['batch_size'], shuffle=False, num_workers=num_workers, worker_init_fn=util_data.seed_worker),
...
for model_name in model_list:
    ...
    model = util_model.initialize_model(model_name=model_name, num_classes=len(classes), feature_extract=cfg['model']['feature_extract'], pretrained=cfg['model']['pretrained'])
